...
* **ms 단위의 시간 분해능:** :  AHRS_getData() 함수는 초당 500에서 800번 이상 실행됩니다. 그런데 ms(밀리초) 단위를 쓰면 dt가 고작 1에서 2 정도로 찍힙니다. 이 거친 해상도로는 적분 오차가 엄청나게 발생해서 정확도가 엉망이 됩니다. timer14 cnt 캡처식 us초로 교체했습니다. 캡처식이 인터럽트식보다 훨씬 신뢰성이 높습니다.
...
* **!RPY 출력이 IMU 자이로를 안쓰고 IMU 가속도만 쓰고있음!:** IMU는 자이로와 가속도를 측정할 수 있습니다. 하지만 현재 코드는 IMU의 가속도만 사용합니다. 드론은 자이로 적분으로 빠른 자세 변동에 대응하고, 가속도(중력 기준)로 장기 드리프트 보정을 하는 상보필터 구조를 사용합니다(칼만필터 미사용시 전부 상보필터를 사용한다고 보면됨). 자이로(각속도)는 진동에 강하나 드리프트가 발생하고 가속도(중력기준)은 절대 기준을 주지만 진동(노이즈)에 매우 민감합니다. IMU는 자이로(Gyroscope)와 가속도(Accelerometer) 두개를 제공합니다. 드론에서 오일러각을 사용시 두개중 하나만 사용하는건 잘못 된 알고리즘입니다. 
...
상보필터는단기적으로는 자이로 적분값(빠르고 매끄러운 응답)을 중심으로 자세를 추정하고, 장기적으로는 가속도 기반 각도(중력 기준의 절대 안정성)를 저주파 성분으로 천천히 섞어 자이로 드리프트를 억제합니다. 오일러각을 통해 드론제어를 하려면 필히 상보필터가 필요합니다.
...
### 1.2 AHRS 추가 개선 사항 (Anti-Drift Logic)
