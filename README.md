...
* **ms 단위의 시간 분해능:** :  AHRS_getData() 함수는 초당 500 '~' 800번 이상 실행됩니다. 그런데 ms(밀리초) 단위를 쓰면 dt가 고작 1 '~' 2 정도로 찍힙니다. 이 거친 해상도로는 적분 오차가 엄청나게 발생해서 정확도가 엉망이 됩니다. timer14 cnt 캡처식 us초로 교체했습니다. 캡처식이 인터럽트식보다 훨씬 신뢰성이 높습니다. 
