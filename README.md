본 프로젝트는 기존에 작성된 코드베이스를 바탕으로 진행되었습니다.  
...
(일단 기존 RC_init은 스로틀이 1000이면 칼리브레이션 과정에 진입을 안한다. 스로틀이 1050이상이고 부저가 울린다음에 1000으로 맞춰줄 때만 칼리브레이션이 일어난다.)
(또한 칼리브레이션 함수도 칼리브레이션을 100퍼센트 보장할 수 없음)
이에 따라,
- RC 구조를 PPM 전용으로 단순화
- 동적 할당 및 불필요한 분기 제거
...
(setPPM에서는 RC 채널 MIN MAX값을 설정해주는 함수가 빠져있음. 그래서 RC_init에서 RC_MSG_setChannelValue(value, i) 부분이 계속해서 진행이 안됨. 진행이 되도 raw값이 0임)
(자세한 내용은 이슈에 각 파트마다 올리겠습니다)
등을 통해 RC 입력을 최대한 안전하게 개선하였다.

여러 RC 프로토콜 지원과 IRQ HANDLER는 **일단 호버링 성공이 보장되는 상황에서** 안전하게 기존의 코드 방식대로 만들 예정이다. 
