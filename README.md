...
* **출력 RPY에 대한 필터 부재로 노이즈 과다(진동 취약):** 기존 구조에서 실제 출력 RPY는 가속도·지자계로 계산한 Euler에 직접 의존했기 때문에, 멀티콥터 진동/가속 환경에서 roll/pitch/yaw가 쉽게 흔들렸습니다. 이 노이즈가 그대로 PID에 입력되면서 특히 D항이 노이즈를 증폭시켜 이륙이 어려워지는 문제가 발생했습니다. 따라서 출력 RPY에 상보필터를 적용하여 자이로의 빠른 응답과 가속도 기반 장기 안정성을 결합합니다. 
