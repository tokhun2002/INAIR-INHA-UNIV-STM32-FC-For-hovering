...
### 3. Yaw 계산 방식 문제 (지자계 센서 제외)
기존 코드에서는 yaw가  
지자계 센서 + IMU roll/pitch 값을 함께 사용해 계산되고 있었다.

그러나 이 방식은 roll/pitch 변화에 따라  
...
또한 드론 제어는 오픈루프 방식으로  
각도를 더하고 빼서 맞추는 구조가 아니다.

대부분의 상용 드론은  
IMU 기반 피드백 제어를 사용하며,  
각 모터의 RPM에 오차가 있더라도  
**제어기가 자세 오차를 기준으로 자동으로 보정**한다.
...
### 정리
이 프로젝트의 모든 수정은  
**“호버링을 가장 먼저 성공시키기 위한 선택”**이다.

복잡한 기능보다 안정성,  
확장성보다 명확한 제어 구조를 우선했다.
