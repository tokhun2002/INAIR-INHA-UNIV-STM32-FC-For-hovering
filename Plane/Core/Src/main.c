...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
//rpy test용 스로틀 50 강제주입
static inline void TEST_VirtualRC_Throttle50_NeutralRPY(void) {

	msg.RC_channels.value[0] = 1300; // thr 50%
	msg.RC_channels.value[1] = 1300;
	msg.RC_channels.value[2] = 1300;
	msg.RC_channels.value[3] = 1300;
}

//volatile uint16_t adc_buf[4];
//static inline void PidRiggTest_RC_ADC(void) {
//
//}
// 모터 출력 테스트용
...
void Minilink_Usec_Calculate(void) {
	uint16_t cnt = (uint16_t) TIM14->CNT;
	uint16_t diff = (uint16_t) (cnt - prev);   // wrap 자동 처리
	msg.servo_output_raw.time_usec += (uint32_t) diff;
	prev = cnt;
}

volatile float dbg_dt_imu_max_us = 0.0f;
volatile float dbg_dt_imu_avg_us = 0.0f;
volatile uint32_t dbg_dt_imu_cnt = 0;
volatile uint32_t dbg_err_imu_cnt = 0;
volatile uint32_t dbg_noterr_imu_cnt = 0;
void DTIMU_Check(void) {
	if (!msg.timing.imu_new_sample)
		return;

	if (dbg_dt_imu_cnt < 100) {
		dbg_dt_imu_max_us = 0;
		dbg_dt_imu_cnt++;
		return;
	}
	float dt_us = msg.timing.dt_imu_s;
	// max
	if (dt_us > dbg_dt_imu_max_us) {
		dbg_dt_imu_max_us = dt_us;
	}
	if (dt_us > 0.03) {
		dbg_err_imu_cnt++;
	} else {
...
int main(void) {
	...
	MX_CAN2_Init();
	MX_USB_DEVICE_Init();
	LL_TIM_EnableCounter(TIM13); // ms
	LL_TIM_EnableCounter(TIM14);  // us
	LL_TIM_EnableCounter(TIM4);  // 타이머 4번 켬. 타이머4번에 채널3이 부저에 연결되어있음.
	BuzzerPlayInit();
	...
	while (1) {
		...
		AHRS_GetData();
		//스로틀값 작으면 그냥 계산 패스 후 0으로 수정. 이거 없으면 스로틀 최대한 내려도 제어기 때문에 모터 돌아감
...
static void MX_ADC1_Init(void) {
	...
	/**ADC1 GPIO Configuration
	 ...
	 PC3   ------> ADC1_IN13
	 */
	GPIO_InitStruct.Pin = V_BATT_Pin | RSSI_Pin | PM_CURRENT_Pin
			| PM_VOLTAGE_Pin;
...
static void MX_TIM5_Init(void) {
	...
	/**TIM5 GPIO Configuration
	 ...
	 PA3   ------> TIM5_CH4
	 */
	GPIO_InitStruct.Pin = MAIN_CH5_Pin | MAIN_CH6_Pin | MAIN_CH7_Pin
			| MAIN_CH8_Pin;
	GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
...
static void MX_USART2_UART_Init(void) {
	...
	/**USART2 GPIO Configuration
	 ...
	 PD6   ------> USART2_RX
	 */
	GPIO_InitStruct.Pin = LL_GPIO_PIN_3 | LL_GPIO_PIN_4 | Telm1_TX_Pin
			| Telm1_RX_Pin;
	GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
...
static void MX_USART3_UART_Init(void) {
	...
	/**USART3 GPIO Configuration
	 ...
	 PD12   ------> USART3_RTS
	 */
	GPIO_InitStruct.Pin = Telm2_TX_Pin | Telm2_RX_Pin | LL_GPIO_PIN_11
			| LL_GPIO_PIN_12;
	GPIO_InitStruct.Mode = LL_GPIO_MODE_ALTERNATE;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
...
static void MX_GPIO_Init(void) {
	...
	/**/
	LL_GPIO_ResetOutputPin(GPIOE,
			LED_BLUE_Pin | GYRO2_NSS_Pin | MAG_NSS_Pin | GYRO1_NSS_Pin
					| BARO_NSS_Pin | LED_RED_Pin | LED_YELLOW_Pin);

	/**/
	LL_GPIO_ResetOutputPin(GPIOC,
			FLASH_NSS_Pin | CAN1_STBY_Pin | CAN2_STBY_Pin);

	/**/
	LL_GPIO_ResetOutputPin(GPS1_SW_LED_GPIO_Port, GPS1_SW_LED_Pin);

	/**/
	GPIO_InitStruct.Pin = LED_BLUE_Pin | GYRO2_NSS_Pin | MAG_NSS_Pin
			| GYRO1_NSS_Pin | BARO_NSS_Pin | LED_RED_Pin | LED_YELLOW_Pin;
